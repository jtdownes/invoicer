...
const inDays = d => new Date(Date.now() + d * 86400000).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
...
export function EstimateBuilder({ onBack }) {
  ...
  async function handleAddClient() {
    ...
    try {
      const client = await post('/api/clients', { name: newName.trim(), email: newEmail || null, phone: newPhone || null })
    ...
    <div className="space-y-6 p-4 md:p-7 overflow-y-auto flex-1">
        ...
        {clientsLoading ? (
          ...
          <div className="bg-gray-50 rounded-xl p-3 space-y-2 border border-gray-200">
            <div className="flex items-center justify-between mb-1">
              ...
              <button onClick={() => { setAddingClient(false); setNewName(''); setNewEmail(''); setNewPhone('') }}
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          <div><div className="text-xs text-gray-400 mb-1">Number</div><input readOnly value="Auto" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-400 bg-gray-50" /></div>
          <div><div className="text-xs text-gray-400 mb-1">Date</div><input readOnly value={today} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-400 bg-gray-50" /></div>
          <div><div className="text-xs text-gray-400 mb-1">Valid (days)</div><input type="number" value={validDays} onChange={e => setValidDays(parseInt(e.target.value) || 30)} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 text-center" /></div>
        </div>
      </div>
          ...
          {items.map(item => (
            <div key={item.id} className="bg-gray-50 rounded-xl p-3 space-y-2">
              ...
              <div className="flex gap-2 items-center">
                <div className="flex-1"><div className="text-xs text-gray-400 mb-1">Qty</div><input type="number" value={item.qty} onChange={e => updItem(item.id, 'qty', e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 bg-white text-center" /></div>
                <div className="flex-1"><div className="text-xs text-gray-400 mb-1">Rate ($)</div><input type="number" value={item.rate} onChange={e => updItem(item.id, 'rate', e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 bg-white" /></div>
                <div className="text-right pt-4"><div className="text-sm font-semibold text-gray-900">{fmt(item.qty * item.rate)}</div><button onClick={() => removeItem(item.id)} className="text-gray-300 hover:text-red-400 mt-1 p-1"><Trash2 className="w-3.5 h-3.5" /></button></div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex items-center justify-between py-1">
        ...
        <div className="flex items-center gap-1.5">
          <input type="number" value={taxRate} onChange={e => setTaxRate(parseFloat(e.target.value) || 0)} className="w-20 border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 text-right" />
      ...
      <div className="w-full max-w-xl">
        ...
        <div className="bg-white rounded-2xl shadow-[0_0_0_1px_rgba(0,0,0,.06),0_4px_6px_-1px_rgba(0,0,0,.07),0_12px_40px_rgba(0,0,0,.06)] p-6 md:p-10">
          ...
          <div className="grid grid-cols-3 bg-gray-50 rounded-xl p-3 mb-5 gap-2">
            <div><div className="text-xs text-gray-400 mb-0.5">Date</div><div className="text-xs font-semibold text-gray-700">{today}</div></div>
            <div><div className="text-xs text-gray-400 mb-0.5">Valid Until</div><div className="text-xs font-semibold text-gray-700">{inDays(validDays)}</div></div>
          ...
          <div className="mb-5">
            ...
            {selectedClient
              ? <div><div className="text-sm font-semibold text-gray-900">{selectedClient.name}</div>{selectedClient.email && <div className="text-xs text-gray-500">{selectedClient.email}</div>}{selectedClient.phone && <div className="text-xs text-gray-500">{selectedClient.phone}</div>}</div>
          ...
          <div className="border-t border-gray-200 pt-3 mt-1 space-y-1.5">
            ...
            {taxRate > 0 && <div className="flex justify-between text-xs text-gray-500"><span>Tax ({taxRate}%)</span><span>{fmt(tax)}</span></div>}
            <div className="flex justify-between text-sm font-bold text-gray-900 pt-2 border-t border-gray-200 mt-1"><span>Estimate Total</span><span className="text-indigo-600">{fmt(total)}</span></div>
          </div>
    ...
    <div className="flex flex-col h-screen overflow-hidden">
      <div className="flex items-center gap-3 px-4 md:px-6 py-3.5 bg-white border-b border-gray-200 flex-shrink-0 sticky top-0 z-20">
        ...
        <div className="flex gap-2">
          <button onClick={() => save(false)} disabled={saving} className="px-3 py-2 text-xs font-semibold text-gray-600 bg-white border border-gray-300 rounded-lg min-h-[38px] disabled:opacity-50">{saving ? 'Saving…' : 'Save Draft'}</button>
          <button onClick={() => save(true)} disabled={saving} className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-white bg-indigo-600 rounded-lg min-h-[38px] disabled:opacity-50"><Send className="w-3 h-3" />{saving ? 'Sending…' : 'Send Estimate'}</button>
        </div>
      </div>
      <div className="md:hidden flex border-b border-gray-200 bg-white flex-shrink-0">
        {['edit', 'preview'].map(t => (
          <button key={t} onClick={() => setTab(t)} className={`flex-1 py-3 text-sm font-semibold capitalize transition-colors ${tab === t ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-gray-400'}`}>
            {t === 'edit' ? 'Edit' : 'Preview'}
          </button>
        ))}
      </div>
