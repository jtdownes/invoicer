...
export function Dashboard({ onNew }) {
  ...
  useEffect(() => {
    get('/api/estimates/dashboard')
      .then(setData)
      .catch(() => setData({ stats: {}, recent_estimates: [] }))
                  ...
                  <div key={est.id} className="px-4 py-4 flex items-center gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-1.5">
                        ...
                        <span className="text-sm font-bold text-gray-900 flex-shrink-0">{fmt(est.total)}</span>
                      </div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <Badge status={est.status} />
                        <span className="text-xs text-gray-400">{fmtDate(est.created_at)}</span>
                        {est.view_count > 0
                          ? <span className="flex items-center gap-0.5 text-xs text-emerald-600 font-medium"><Eye className="w-3 h-3" />{est.view_count} views</span>
